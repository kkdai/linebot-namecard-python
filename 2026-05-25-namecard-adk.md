...
![ADK Namecard](../images/Finder%202026-04-16%2021.43.57.png)
...
如果 Vertex AI 或 ADK 在執行中發生 any 異常，系統會自動在背景啟用 Firebase 本地關鍵字比對，依然能完美回傳相匹配的名片 Flex 訊息，給予使用者最優雅的防護網：
...
# 特別加碼：Antigravity CLI (agy) 配色看不見字的救星！

在這次重構與開發的過程中，我們使用了強大的 agentic AI 命令行助手 **Antigravity CLI (agy)** 進行配對編程與日誌追蹤。

但許多人在 macOS 終端機（預設為深色背景）啟動 agy 時，會遇到**「字體黑成一片，完全看不清文字」**的痛苦對比度問題。

這是因為 agy 預設設定檔可能被配置成了淺色主題。我們不需要妥協，只需要修改 `settings.json` 即可一鍵得救！
...
完整且經 linter 優化的程式碼已同步推播至 [GitHub](https://github.com/kkdav/linebot-namecard-python)。希望這篇動態閉包設計與 Cloud Run、Event Loop 填坑實戰，能幫助大家在打造生產級 AI Agent Web 應用時少走更多彎路！我們下期見！
