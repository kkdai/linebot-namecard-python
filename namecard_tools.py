...
if gemini_api_key:
    ...
    print("GEMINI_API_KEY not found in environment variables. Gemini tools may not function.")

# Configure LINE Channel Access Token
CHANNEL_ACCESS_TOKEN = os.getenv("ChannelAccessToken")
if not CHANNEL_ACCESS_TOKEN:
    print("ChannelAccessToken not found in environment variables. LINE tools may not function.")
...
def add_namecard_tool(user_id: str, namecard_data: dict) -> str:
    ...
    """
    if not user_id: return "Failed: user_id empty."
    if not namecard_data or not isinstance(namecard_data, dict): return "Failed: namecard_data invalid."
    try:
        ref = db.reference(f"{NAMECARD_DB_PATH}/{user_id}")
...
def remove_redundant_data_tool(user_id: str) -> str:
    ...
    try:
        ref = db.reference(f"{NAMECARD_DB_PATH}/{user_id}")
        namecard_data = ref.get()
        if not namecard_data: return "No namecards found."
        email_map, keys_to_delete = {}, []
        for key, value in namecard_data.items():
            email = value.get("email")
            if email:
                if email in email_map: keys_to_delete.append(key)
                else: email_map[email] = key
        if not keys_to_delete: return "No redundant namecards found."
        for key in keys_to_delete: ref.child(key).delete()
        return f"Redundant data removal complete. {len(keys_to_delete)} card(s) removed."
    except Exception as e:
    ...
    try:
        ...
        if all_cards:
            for card_details in all_cards.values():
                if card_details.get("email") == email_to_check: return True
        return False
    except Exception as e:
        print(f"Error checking if namecard exists for {user_id} with email {namecard_data.get('email')}: {e}")
        return False
...
def parse_namecard_from_image_tool(image_bytes: bytes) -> dict:
    ...
    if not image_bytes: return {"error": "Image bytes cannot be empty."}
    try: img = PIL.Image.open(BytesIO(image_bytes))
    except Exception as e: return {"error": f"Failed to open image: {e}"}
    try:
        model_name = os.getenv("GEMINI_MODEL", "gemini-1.5-flash-latest")
        model = genai.GenerativeModel(model_name, generation_config={"response_mime_type": "application/json"})
        response = model.generate_content([IMGAGE_PROMPT, img], stream=True); response.resolve()
        if not response.text: return {"error": "Gemini returned empty response."}
        card_obj = json.loads(response.text)
        return {k.lower(): v for k, v in card_obj.items()}
    except json.JSONDecodeError as e:
        return {"error": "Failed to parse JSON from Gemini.", "details": response.text if 'response' in locals() else "No response text"}
    except Exception as e:
        error_details = str(e)
        if 'response' in locals() and hasattr(response, 'prompt_feedback'): error_details += f" | PF: {response.prompt_feedback}"
...
def query_namecards_tool(user_id: str, user_query: str, all_cards_list: list) -> list:
    ...
    """
    if not gemini_api_key: print("Gemini key not configured."); return []
    if not user_query or not all_cards_list: return []
    try:
        ...
        formatted_prompt = QUERY_PROMPT.format(all_cards=all_cards_json_str, msg=user_query)
        model_name = os.getenv("GEMINI_MODEL", "gemini-1.5-flash-latest")
        model = genai.GenerativeModel(model_name, generation_config={"response_mime_type": "application/json"})
        response = model.generate_content([{"role": "user", "parts": [formatted_prompt]}])
        if not response.text: print("Gemini query response empty."); return []
        parsed_response = json.loads(response.text)
        if isinstance(parsed_response, dict): return [parsed_response]
        elif isinstance(parsed_response, list): return parsed_response
        else: print(f"Unexpected Gemini query response format: {type(parsed_response)}"); return []
    except json.JSONDecodeError as e:
        print(f"JSON parsing failed for query: {e}. Response: {response.text if 'response' in locals() else 'No response'}")
        return []
    except Exception as e:
        error_details = str(e)
        if 'response' in locals() and hasattr(response, 'prompt_feedback'): error_details += f" | PF: {response.prompt_feedback}"
        print(f"Error querying namecards with Gemini for user {user_id}: {error_details}"); return []
            ...
            "contents": [
                {
                    ...
                    "flex": 1,
                    "size": "full",
                    "url": card_data.get("image_url", "https://raw.githubusercontent.com/kkdai/linebot-smart-namecard/main/img/logo.jpeg"),
                },
                {
                    "type": "box",
                    "flex": 4,
                    "layout": "vertical",
                    "contents": [
                        {"type": "text", "align": "end", "size": "xxl", "text": str(card_data.get('name', 'N/A')), "weight": "bold"},
                        {"type": "text", "align": "end", "size": "sm", "text": str(card_data.get('title', 'N/A'))},
                        {"type": "text", "align": "end", "margin": "xxl", "size": "lg", "text": str(card_data.get('company', 'N/A')), "weight": "bold"},
                        {"type": "text", "align": "end", "size": "sm", "text": str(card_data.get('address', 'N/A'))},
                        {"type": "text", "align": "end", "margin": "xxl", "text": str(card_data.get('phone', 'N/A'))},
                        {"type": "text", "align": "end", "text": str(card_data.get('email', 'N/A'))},
                    ],
                },
            ],
        },
...
def send_text_message_tool(user_id: str, message_text: str) -> str:
    ...
    """
    if not CHANNEL_ACCESS_TOKEN: return "Failed: ChannelAccessToken not configured."
    if not user_id: return "Failed: user_id empty."
    if not message_text: return "Failed: message_text empty."
    try:
...
def send_flex_message_tool(user_id: str, alt_text: str, namecard_data_for_flex: dict) -> str:
    """
    ...
    if not CHANNEL_ACCESS_TOKEN: return "Failed: ChannelAccessToken not configured."
    if not user_id: return "Failed: user_id empty."
    if not alt_text: return "Failed: alt_text empty."
...
if __name__ == '__main__':
    ...
    if not CHANNEL_ACCESS_TOKEN:
        ...
        if test_user_id != "test_user_tools_123":
            text_send_status = send_text_message_tool(test_user_id, f"Test from namecard_tools.py: This is a sample card: {sample_card.get('name')}")
            print(f"Send text message status: {text_send_status}")
        else:
            print("Skipping actual send_text_message_tool call to dummy user_id for sample card text.")
        ...
        if test_user_id != "test_user_tools_123" and sample_card:
            ...
            flex_send_status = send_flex_message_tool(test_user_id, f"Sample Card: {sample_card.get('name')}", sample_card)
            print(f"Send flex message (sample card) status: {flex_send_status}")
        else:
            print("Skipping actual send_flex_message_tool call to dummy user_id for sample card flex.")
